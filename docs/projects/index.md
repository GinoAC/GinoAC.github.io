---
title: Projects
---

## Current Projects

### An Impractical Approach to Understanding Microarchitectural Predictors

### Homebrew Compute Cluster

## Projects

### Prefetching

#### Software Instruction Prefetching

### Cache Coherence

### Coherence Security

### Active Interposers as the Root of Trust

### ChampSim

---

## Publications

**ORAP: Optimized Row Access Prefetching for Rowhammer-mitigated Memory**  
M Merrell, D Puckett, G Chacon, J Stuecheli, S Kalafatis, PV Gratz  
*arXiv preprint*, 2026

**Skia: Exposing Shadow Branches**  
C Pepi, BR Godala, K Tibrewala, GA Chacon, PV Gratz, DA Jiménez, et al.  
*30th ACM International Symposium on Architectural Support for Programming Languages and Operating Systems (ASPLOS)*, 2025

**Correct Wrong Path**  
BR Godala, SP Ramesh, K Tibrewala, C Pepi, G Chacon, S Kanev, et al.  
*IEEE Computer Architecture Letters*, 2025

**R-Max: A Method for Approximating the Benefit of Ideal Prefetching and Replacement Policy**  
L Wang, CH Lee, M Merrell, G Chacon, DA Jiménez, PV Gratz  
*IEEE Computer Architecture Letters*, 2025

**Coherence Attacks and Countermeasures in Interposer-Based Chiplet Systems**  
GA Chacon, C Williams, J Knechtel, O Sinanoglu, PV Gratz, V Soteriou  
*ACM Transactions on Architecture and Code Optimization*, 2024

**A Characterization of the Effects of Software Instruction Prefetching on an Aggressive Front-End**  
G Chacon, N Gober, K Nathella, PV Gratz, DA Jiménez  
*IEEE International Symposium on Performance Analysis of Systems and Software (ISPASS)*, 2023

**Processor Memory System Design for Performance and Security** *(Dissertation)*  
GA Chacon  
*Texas A&M University*, 2023

**The Championship Simulator: Architectural Simulation for Education and Competition**  
N Gober, G Chacon, L Wang, PV Gratz, DA Jimenez, E Teran, S Pugsley, et al.  
*arXiv preprint arXiv:2210.14324*, 2022

**Page Size Aware Cache Prefetching**  
G Vavouliotis, G Chacon, L Alvarez, PV Gratz, DA Jiménez, M Casas  
*55th IEEE/ACM International Symposium on Microarchitecture (MICRO)*, 2022

**Hardware Trojan Threats to Cache Coherence in Modern 2.5D Chiplet Systems**  
GA Chacon, C Williams, J Knechtel, O Sinanoglu, PV Gratz  
*IEEE Computer Architecture Letters*, 2022

**Composite Instruction Prefetching**  
G Chacon, E Garza, A Jimborean, A Ros, PV Gratz, DA Jiménez, et al.  
*40th IEEE International Conference on Computer Design (ICCD)*, 2022

**Temporal Ancestry Prefetcher**  
N Gober, G Chacon, DA Jiménez, P Gratz  
*1st Instruction Prefetching Championship (IPC1)*, 2020

**Barca: Branch Agnostic Region Searching Algorithm**  
DA Jiménez, PV Gratz, GA Chacon, N Gober  
*1st Instruction Prefetching Championship (IPC1)*, 2020

**Perceptron-Based Prefetch Filtering**  
E Bhatia, G Chacon, S Pugsley, E Teran, PV Gratz, DA Jiménez  
*46th International Symposium on Computer Architecture (ISCA)*, 2019

**Enhancing Signature Path Prefetching with Perceptron Prefetch Filtering**  
E Bhatia, G Chacon, E Teran, PV Gratz, DA Jiménez  
*Data Prefetching Championship (DPC3)*, 2019

---

## Patents

**Providing Dead-Block Prediction for Determining Whether to Cache Data in Cache Devices**  
AR Alameldeen, G Chacon  
US Patent 10,877,890 — 2020

**Adaptive Granularity for Reducing Cache Coherence Overhead**  
G Chacon, AR Alameldeen  
US Patent 10,691,602 — 2020
